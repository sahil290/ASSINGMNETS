...
const changeColor = document.getElementById('quote-btn'),
    colors = ['red', 'green', 'blue', 'grey', 'white', 'black', 'darkblue', 'darkOrange'];
let colorIndex = 0;
changeColor.addEventListener('click', () => {
    document.body.style.backgroundColor = colors[colorIndex];
    colorIndex = (colorIndex + 1) % colors.length;
});
/* BUTTON BACKGROUND COLOR CHANGING CODE */
document.getElementById("quote-btn").onclick = function () {
    document.getElementById("quote-btn").style.backgroundColor = colors[colorIndex];
    colorIndex = (colorIndex + 1) % colors.length;
}
